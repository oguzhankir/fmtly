...
type ZodRoot = 'AutoGeneratedSchema';
...
function isJsonTypeArray(type: JsonSchema['type']): type is JsonSchemaType[] {
...
function toZodOptional(expr: string): string {
	// Zod optional works as a chained modifier: <schema>.optional()
...
function schemaToZodExpression(schema: JsonSchema, level: number): string {
	if (!schema || (Object.keys(schema).length === 0 && schema.constructor === Object)) {
		return 'z.any()';
	}

	if (schema.anyOf?.length) {
		const variants = schema.anyOf.map((s) => schemaToZodExpression(s, level + 1));
		const ind = indent(level + 1);
		const closingInd = indent(level);
		return `z.union([\n${variants.map((v) => `${ind}${v}`).join(',\n')}\n${closingInd}])`;
	}

	if (isJsonTypeArray(schema.type)) {
		const variants = schema.type.map((t) => schemaToZodExpression({ type: t }, level + 1));
		const ind = indent(level + 1);
		const closingInd = indent(level);
		return `z.union([\n${variants.map((v) => `${ind}${v}`).join(',\n')}\n${closingInd}])`;
	}

	const type = schema.type as JsonSchemaType | undefined;
	if (!type) {
		if (schema.properties) return schemaToZodExpression({ type: 'object', ...schema }, level);
		if (schema.items) return schemaToZodExpression({ type: 'array', ...schema }, level);
		return 'z.any()';
	}
	...
	switch (type) {
			...
			const inner = keys.length
				? keys
						.map((key) => {
							...
							const expr = schemaToZodExpression(propSchema, level + 1);
							const propExpr = required.has(key) ? expr : toZodOptional(expr);
							return `${indent(level + 1)}${JSON.stringify(key)}: ${propExpr}`;
						})
						.join(',\n')
				: '';

			const strict = schema.additionalProperties === false ? '.strict()' : '';
			if (!inner) return `z.object({})${strict}`;
			return `z.object({\n${inner}\n${indent(level)}})${strict}`;
		}
		case 'array': {
			const items =
				schema.items && Object.keys(schema.items).length > 0
					? schemaToZodExpression(schema.items, level + 1)
					: 'z.any()';
		...
		case 'string': {
			...
			if (schema.enum) {
				const exprs = schema.enum
					.map((v) => `z.literal(${v === null ? 'null' : JSON.stringify(v)})`)
					.join(', ');
		...
		case 'number': {
			if (schema.enum && schema.enum.length > 0) {
				const exprs = schema.enum
					.map((v) => `z.literal(${v === null ? 'null' : String(v)})`)
					.join(', ');
				return `z.union([${exprs}])`;
			}
			return 'z.number()';
		...
		case 'boolean': {
			if (schema.enum && schema.enum.length > 0) {
				const exprs = schema.enum
					.map((v) => (typeof v === 'boolean' ? `z.literal(${v})` : null))
					.filter((v): v is string => v !== null)
					.join(', ');
				if (exprs) return `z.union([${exprs}])`;
			}
			return 'z.boolean()';
		}
		case 'null': {
...
export function toZodSchema(json: string): string {
	const parsed = parseJSON(json);
	if (!parsed.success) {
		throw new Error(parsed.error.plainLanguageExplanation);
	}
	...
	const inferred = inferSchemaFromValues([parsed.data as JsonValue]);
	const rootExpr = schemaToZodExpression(inferred, 0);

	const rootName: ZodRoot = 'AutoGeneratedSchema';
	return `import { z } from 'zod';

export const ${rootName} = ${rootExpr};

export type AutoGenerated = z.infer<typeof ${rootName}>;
`;
}

export function parseNdjson(input: string, indentLevel = 2): NdjsonParseResult {
	if (!input.trim()) {
		...
		if (empty.success) {
			return {
				...
				error: {
					...
					column: 1,
					position: 0,
					plainLanguageExplanation: 'Input is empty'
				}
			};
		}
		return { success: false, error: empty.error };
	}
	...
	for (let index = 0; index < rawLines.length; index += 1) {
		...
		if (parsed.success) {
			entries.push({
				...
				formatted: JSON.stringify(parsed.data, null, indentLevel)
			});
			continue;
		}
		...
		const error: ParseError = {
			...parsed.error,
			line: lineNumber,
			plainLanguageExplanation: parsed.error.plainLanguageExplanation
		};
	...
	if (entries.length === 0) {
		return {
			success: false,
			error: {
				message: 'Empty NDJSON input',
				line: 1,
				column: 1,
				position: 0,
				plainLanguageExplanation: 'The NDJSON input is empty.'
			}
		};
	}
...
export function formatNdjsonReport(input: string, indentLevel = 2): string {
	const parsed = parseNdjson(input, indentLevel);
	if (!parsed.success) return '';

	const report = parsed.entries.map((entry) => {
		if (entry.kind === 'value') {
