				...
				<div class="flex flex-wrap gap-[var(--space-2)]">
					{#each variantOptions as option}
						<button
							...
							class={`inline-flex h-8 items-center justify-center rounded-[var(--radius-md)] border px-[var(--space-3)] text-[length:var(--text-xs)] font-[number:var(--weight-semibold)] ${getChoiceButtonClass(selectedVariant === option.value)}`}
						>
							{$t(option.labelKey, option.value)}
						</button>
					{/each}
