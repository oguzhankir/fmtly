<script lang="ts">
	...
	import { removeDuplicateLines, type TextDuplicateRemovalResult } from '$engines/text/index.js';
	...
	let result = $derived(removeDuplicateLines($input));
	...
	<div class="grid h-full min-h-0 grid-cols-1 md:grid-cols-2">
		<div class="flex min-h-0 flex-col border-r border-[var(--border-default)] bg-[var(--bg-base)]">
			...
			<textarea
				...
				placeholder={$t('ui.duplicate_line_remover.placeholder', 'Type or paste text to remove duplicate lines...')}
				spellcheck="false"
			></textarea>
		</div>

		<div class="flex min-h-0 flex-col overflow-y-auto bg-[var(--bg-surface)] p-[var(--space-3)]">
			<div class="mb-[var(--space-3)] grid grid-cols-2 gap-[var(--space-2)]">
				<div class="rounded-[var(--radius-md)] border border-[var(--border-default)] bg-[var(--bg-base)] p-[var(--space-2)]">
					<div class="text-[length:var(--text-xs)] text-[var(--text-tertiary)]">{$t('ui.duplicate_line_remover.unique_lines', 'Unique lines')}</div>
					<div class="text-[length:var(--text-lg)] font-[number:var(--weight-semibold)] text-[var(--text-primary)]">{result.uniqueCount.toLocaleString()}</div>
				</div>
				<div class="rounded-[var(--radius-md)] border border-[var(--border-default)] bg-[var(--bg-base)] p-[var(--space-2)]">
					<div class="text-[length:var(--text-xs)] text-[var(--text-tertiary)]">{$t('ui.duplicate_line_remover.duplicates_removed', 'Duplicates removed')}</div>
					<div class="text-[length:var(--text-lg)] font-[number:var(--weight-semibold)] text-[var(--text-primary)]">{result.duplicateCount.toLocaleString()}</div>
				</div>
			</div>
			...
			<div class="flex-1 min-h-0">
				<div class="mb-[var(--space-2)] flex items-center justify-between gap-[var(--space-2)]">
					<h3 class="text-[length:var(--text-xs)] font-[number:var(--weight-semibold)] text-[var(--text-secondary)]">
						{$t('ui.duplicate_line_remover.result', 'Result')}
					</h3>
					<button
						type="button"
						onclick={() => copyValue(result.removed)}
				...
				<div class="h-full max-h-96 overflow-y-auto whitespace-pre-wrap break-all rounded-[var(--radius-sm)] border border-[var(--border-subtle)] bg-[var(--bg-surface)] px-[var(--space-2)] py-[var(--space-2)] font-[family-name:var(--font-mono)] text-[length:var(--text-sm)] text-[var(--text-primary)]">
					{result.removed}
				</div>
			</div>
		</div>
