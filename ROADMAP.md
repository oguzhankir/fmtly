...
| 107 | System Prompt Builder ✅ | `ai/system-prompt` | done — Guided browser-only builder for production-ready system prompts with templates for code assistant, data analyst, translator, and customer support; structured constraints, examples, safety boundaries, quality checks, Web Worker support for large inputs, and export as plain text or OpenAI JSON message array. |
...
| 139 | Image Compressor | `image/compress` | done — Reduce file size with configurable quality, smart auto-format selection, Web Worker processing for large images, and real-time before/after preview. |
