...
import type { DiffEntry, DiffOptions } from '../../../src/lib/engines/json/diff.js';
import { computeTOMLDiff } from '../../../src/lib/engines/toml/diff.js';
...
describe('computeTOMLDiff', () => {
	it('detects modified TOML values', async () => {
		...
		const modified = result.entries.filter((entry: DiffEntry) => entry.type === 'modified');
	...
	it('detects added TOML keys', async () => {
		...
		expect(result.error).toBeNull();
		const added = result.entries.filter((entry: DiffEntry) => entry.type === 'added');
